...
export default function DashboardPage() {
  ...
  // Realtime: changes will trigger refresh
  React.useEffect(() => {
    if (!org) return;
    const channel = supabase
      .channel("rt-dashboard-" + org.id)
      .on("postgres_changes", { event: "*", schema: "public", table: "sales_orders", filter: `org_id=eq.${org.id}` }, () => refresh())
      .on("postgres_changes", { event: "*", schema: "public", table: "inventory_batches", filter: `org_id=eq.${org.id}` }, () => refresh())
      .on("postgres_changes", { event: "*", schema: "public", table: "payments", filter: `org_id=eq.${org.id}` }, () => refresh())
      .subscribe();
